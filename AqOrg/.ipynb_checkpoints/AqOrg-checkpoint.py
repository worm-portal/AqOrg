...
#             raise Exception("group_data must be a CSV file.")
        ...
        if test:
            ...
            elif state == 'gas' and ig_method=="Joback":
                self.__estimate_joback()
            elif state == 'gas' and ig_method=="custom":
                self.__estimate_custom()
    ...
    #CUSTOM
    def __est_custom(self):
        
        #I have the material point for Cph hard coded as 0 to workaround an error. Think about this later if it should change!
        
        # values to be added to final estimate of each property
        custom_props = {"Gig":0, "Hig":0, # kJ/mol
                        "Cpig":0, "Sig":0} # j/mol/K
        
        for prop in custom_props.keys():
            mol_prop = 0
            error_groups = []
            ...
            for group in self.groups:
                ...
                if len(error_groups) == 0:
                    self.__setattr__(prop, mol_prop+custom_props[prop])
                else:
                    msg = self.name + " encountered errors with group(s): " +\
                        str(error_groups) + ". Are these groups assigned "+\
                        "ideal gas properties in the custom data file?"
                    raise Exception(msg)
...
#                     mol_err = format(mol_err, '.'+str(n_dec)+'f')
        ...
        ## ESTIMATE IDEAL GAS PROPERTIES BASED ON CUSTOM DATABASE
        elif self.ig_method == "custom":
            # estimation of the Gibbs free energy of formation of the
            # ideal gas (Joule-based) from linear regression estimation.
            try:
                custom_estimate = custom(self.name)
                
                if self.Gig == None:
                    self.Gig = custom_estimate["Gig"]
                if self.Hig == None:
                    self.Hig = custom_estimate["Hig"]
                if self.Sig == None:
                    self.Sig = custom_estimate["Sig"]
                if self.Cpig == None:
                    self.Cpig = custom_estimate["Cpig"]
            except:
                ig_gas_error = True
    ...
    def __estimate_custom(self): #NEW
        self.__set_groups()
        self.__est_custom()
...
###ESTIMATE IDEAL GAS PROPERTIES WITH CUSTOM DATABASE
def custom(name):
    
    """
    Estimate standard state ideal gas properties of a molecule using custom datasheet
    ...
    ig_est = Estimate(name, state='gas', show=False, ig_method="custom",
                      group_data=pkg_resources.resource_stream(__name__, 'data/estimated_ideal_gas_groups.csv'), index_col="groups")
    
    ### FUCKING AROUND HERE
    ig_data = pd.read_csv(pkg_resources.resource_stream(__name__, 'data/estimated_ideal_gas_groups.csv'))
    ind = ig_data.loc[ig_data['group'] == 'Yo'].index[0]  #[['Gig','Hig','Sig','Cpig']]
    mp_gas = ig_data.loc[ind, ['Gig','Hig','Sig','Cpig']]
#     print(type(ig_props['Gig']))
    
    return {'Gig':ig_est.Gig+mp_gas['Gig'], 'Hig':ig_est.Hig+mp_gas['Hig'], #these are needed for estimation of aqueous things...
            'Sig':ig_est.Sig+mp_gas['Sig'], 'Cpig':ig_est.Cpig+mp_gas['Cpig']} 

    ####

#     return {'Gig':ig_est.Gig, 'Hig':ig_est.Hig, #these are needed for estimation of aqueous things...
#                 'Sig':ig_est.Sig, 'Cpig':ig_est.Cpig} 